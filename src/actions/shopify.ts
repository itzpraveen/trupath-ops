...
import { z } from "zod";
import { db } from "@/db";
import { dispatches, dispatchItems, products, shopifyOrders } from "@/db/schema";
...
export async function fulfilOrderInShopify(_prev: ActionState, formData: FormData): Promise<ActionState> {
  try {
    ...
    if (!parsed.ok) return { error: parsed.error, fieldErrors: parsed.fieldErrors };
    const d = parsed.data;
    const [dispatch] = await db.select().from(dispatches).where(eq(dispatches.shopifyOrderId, d.id));
    if (!dispatch || dispatch.status !== "shipped" || !dispatch.qualityCheckedAt || !dispatch.billingCheckedAt) throw new Error("Complete QC, billing, packing and shipping on the dispatch page first");
    const items = await db.select({variantId: products.shopifyVariantId, qty: dispatchItems.qty}).from(dispatchItems).innerJoin(products, eq(products.id, dispatchItems.productId)).where(eq(dispatchItems.dispatchId, dispatch.id));
    const r = await fulfillShopifyOrder(d.id, { company: d.courier, number: d.trackingNo, url: d.trackingUrl }, d.notifyCustomer, items);
