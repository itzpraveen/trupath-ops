...
export function DispatchForm({
  ...
  date: string;
  initial?: Dispatch & { items: { productId: string; qty: number }[] };
  itemsLocked?: boolean;
}) {
  const [lines, setLines] = useState<Line[]>(() => (initial?.items.length ? initial.items.map((it, i) => ({ key: i, productId: it.productId, qty: String(it.qty) })) : [{ key: 0, productId: "", qty: "1" }]));
  const [recordSale, setRecordSale] = useState(false);
  const update = (key: number, patch: Partial<Line>) => setLines((ls) => ls.map((l) => (l.key === key ? { ...l, ...patch } : l)));
  const totalPcs = lines.reduce((s, l) => s + (Number(l.qty) || 0), 0);

  return (
    <ActionForm action={initial ? updateDispatch : createDispatch} submitLabel={initial ? "Save changes" : "Create dispatch"} redirectTo={(s) => (initial ? `/dispatch/${initial.id}` : `/dispatch/${s.id}`)}>
      {(state) => {
              ...
              <div className="space-y-4">
                <div className="space-y-2 rounded-xl border bg-card p-4">
                  <div className="flex items-center justify-between">
                    ...
                    {!itemsLocked ? (
                      <Button type="button" variant="outline" size="xs" onClick={() => setLines((ls) => [...ls, { key: Date.now(), productId: "", qty: "1" }])}>
                        <Plus /> Add item
                      </Button>
                    ) : null}
                  </div>
                  ...
                  <div className="space-y-2">
                    {lines.map((l) => (
                      <div key={l.key} className="grid grid-cols-[minmax(0,1fr)_5rem_2rem] items-start gap-2">
                        {itemsLocked ? (
                          <>
                          ...
                          <ProductPicker products={products} name="productId[]" defaultValue={l.productId} onChange={(p) => update(l.key, { productId: p?.id ?? "" })} required />
                        )}
                ...
                <div className="space-y-4 rounded-xl border bg-card p-4">
                  ...
                  <FormRow>
                    <Field label="Customer name" name="customerName" error={fe.customerName} required>
                      <Input id="customerName" name="customerName" defaultValue={initial?.customerName ?? ""} required list="dispatch-customers" />
                    ...
                    <Field label="Phone" name="phone" error={fe.phone}>
                      <Input id="phone" name="phone" inputMode="tel" defaultValue={initial?.phone ?? ""} />
                    </Field>
                  </FormRow>
                  ...
                  <Field label="Saved customer (for statements)" name="contactId" error={fe.contactId}>
                    <NativeSelect id="contactId" name="contactId" defaultValue={initial?.contactId ?? ""}>
              ...
              <div className="space-y-4">
                <div className="space-y-4 rounded-xl border bg-card p-4">
                  ...
                  <Field label="Order value (₹)" name="amountP" error={fe.amountP}>
                    <Input id="amountP" name="amountP" inputMode="decimal" defaultValue={initial?.amountP ? toRupees(initial.amountP) : ""} />
                  </Field>
                  {!initial ? (
                    <>
