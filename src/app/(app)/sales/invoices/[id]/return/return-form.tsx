...
export function ReturnForm({invoice,previous,requestId}:{invoice:Invoice;previous:CreditTotals[];requestId:string}) {
  ...
  return <ActionForm action={createCreditNote} submitLabel="Issue credit note" redirectTo={s=>`/print/credit-note/${s.id}`}>
    {state=><>
      ...
      <p className="text-sm">Record goods physically received back. Enter the quantity fit for sale separately; damaged goods do not go back into saleable stock. Any cash or bank refund is recorded separately in Payments.</p>
      {invoice.lines.map((l,i)=>{const remaining=l.qty-previous.flatMap(p=>p.lines).filter(p=>p.originalLine===i).reduce((n,p)=>n+p.qty,0);return <div key={i} className="grid items-end gap-3 rounded-lg border p-3 sm:grid-cols-[1fr_8rem_8rem]"><div><p className="font-medium">{l.description}</p><p className="text-sm text-muted-foreground">{remaining} remaining · original GST {l.ratePct}%</p></div><label className="text-sm">Returned quantity<Input name="qty[]" aria-label={`Return ${l.description}`} type="number" min={0} max={remaining} step={1} value={quantities[i]} onChange={e=>setQuantities(update(quantities,i,e.target.value))}/></label><label className="text-sm">Fit for sale<Input name="restockQty[]" aria-label={`Restock ${l.description}`} type="number" min={0} max={quantities[i]} step={1} value={stock[i]} onChange={e=>setStock(update(stock,i,e.target.value))}/></label></div>})}
